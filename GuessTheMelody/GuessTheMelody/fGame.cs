...
namespace GuessTheMelody
{
    public partial class fGame : Form
    {
        ...
        private void fGame_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (players[0]==false && e.KeyData == Keys.A)
            {
                ...
                if(/*MessageBox.Show("Правильный ответ?","Игрок 1", MessageBoxButtons.YesNo)*/
                    fm.ShowDialog()==DialogResult.Yes)
                {
                    lbCounter_1.Text = Convert.ToString(Convert.ToInt32(lbCounter_1.Text) + 1);
                    MakeMusic();
                }
                GamePlay();
            }
            if (players[1]==false && e.KeyData == Keys.P)
            {
                ...
                if (/*MessageBox.Show("Правильный ответ?", "Игрок 2", MessageBoxButtons.YesNo)*/ 
                    fm.ShowDialog()== DialogResult.Yes)
                {
                    lbCounter_2.Text = Convert.ToString(Convert.ToInt32(lbCounter_2.Text) + 1);
                    MakeMusic();
                }
                GamePlay();
            }
        ...
        private void lbCounter_1_MouseClick(object sender, MouseEventArgs e)
        {
        ...
        private void lbCounter_2_MouseClick(object sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left)
                (sender as Label).Text = Convert.ToString(Convert.ToInt32((sender as Label).Text) + 1);
            if (e.Button == MouseButtons.Right)
